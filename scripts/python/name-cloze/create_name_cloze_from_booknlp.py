...
def read_ents(filename, output_file):
    ...
    for i in single_per_ents:
        ...
        if not invalid:
            ...
            tokens[i] = " [MASK]" if tokens[i].startswith(" ") else "[MASK]"
            if end - start > min_context and end - start < max_context:
                result_line = "%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
                    idd, end - start, i, start, end, orig.lstrip().rstrip(), ''.join(tokens[start:end + 1]).lstrip().rstrip()
                )
                results.append(result_line)  # Append to results list
...
if __name__ == "__main__":
    read_toks(sys.argv[2])
    output_file_path = sys.argv[3]  # Get output file path from arguments
    read_ents(sys.argv[1], output_file_path)
